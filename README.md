...
- 🗺️ 選單是**真的畫出來的一張島嶼地圖**（`world-map.svg`）：海、島、沙灘、湖、
  一條蜿蜒的小路，七個地方各有自己的插圖（小木屋、向日葵籬笆、大樹和長椅、
  店面街、霓虹招牌的遊戲中心、摩天輪和帳篷、椰子樹和遮陽傘）。
  順序就是遊戲裡走的順序：
  ...
  現在站的地方（紫圈 + 🐹）、還沒解鎖的鎖頭。座標寫在 `WORLD_MAP_PINS`，
  是 SVG 裡那七個景點的中心換算成百分比（原圖 420×700）——
  **改圖的時候這張表要跟著改**，不然名牌會跟景點對不起來。
