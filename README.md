...
| `cedict.js` | 4.1 MB（gzip 1.5 MB），60,903 個中文詞、63,370 個英文查詢鍵，由 CC-CEDICT 建置（CC BY-SA 4.0） |
...
4. **詞頻門檻是用來排序的，不是用來決定收錄的。** 這個混淆害 `pangolin`
   查不到：穿山甲的中文詞頻 2.26、英文 2.36，兩道門檻都砍。但動植物專名
   在兩種語言裡都天生罕見，而且**毫無歧義**——沒有東西要排序，濾掉是純損失。
   現在全部候選都沒過門檻時會至少留最好的一個。
