...
柵欄後探頭的小動物以前只是背景動畫。現在牠們是朋友：可以送禮物、有好感度、
五顆愛心之後會每天回送東西。

- **牠們改成一直看得到**。原本是 17 秒才探頭兩秒（`pointer-events: none`），
  很可愛，但要在那兩秒內點中 —— 對小二生只是挫折。現在在柵欄後一直上下晃。
- 每隻有自己「最喜歡的禮物」，送對好感度加倍。四種禮物 🌰5 / 🍪12 / 🎀25 / 💐40。
- 一天一隻只能送一次（`farmDay()`，和菜園同一套）。升一顆愛心給 `愛心數×15` 金幣，
  那是送禮的主要報酬。
- **三顆愛心以上的朋友每天會自己回送金幣**（進地圖時）。這是「常常回來看看」
  的第二個理由（第一個是菜園），而且不用她做什麼，打開來就有好消息。
- 資料存 `petData.friends`，key 直接用 emoji，所以地圖設定裡的 `peekers` 不用改。

#### 公佈欄與每日挑戰（`QUEST_POOL`）

街上立了一座公佈欄，走過去點它就打開今日挑戰。

- 以前每日任務只能從角落那顆 📜 打開，而且永遠是同三件事（拼字 10 個、
  洗澡 1 次、餵食 3 次），獎勵 2~5 金幣 —— 小到不值得做。
- 現在每天從 11 件的池子裡抽三件，涵蓋世界裡每一種玩法（種菜、收成、送禮、
  坐設施、走訪地圖、玩遊戲、跑輪…），獎勵 6~16 金幣，**三件全做完再給 30**。
- **用日期當亂數種子**（`questRand(farmDay())`），同一天抽到的一定一樣。
  不這樣做的話每次重整都換一批，做到一半的進度就沒意義了。
- 改了題庫記得把 `QUEST_VERSION` +1，當天也會立刻換成新的。
- 「走訪 N 張地圖」算的是**不同的地圖**（`quests.seen`），不然在兩張之間
  來回跑十次就破關了。
- 公佈欄上的紅點 = 還有幾件沒做完，做完變綠勾。
...
**②ʙ 想換成真的音樂？** `music/<地圖>.mp3`（或 `.ogg`）放進去就會自動用音檔，
沒放就照舊即時演奏，兩邊可以混著用。細節和素材來源看 `music/README.md`。
`musicPlay()` 會先讓即時演奏頂著、同時去抓檔案，抓到再無縫換手 ——
一進地圖就等檔案的話會有一兩秒空白。
