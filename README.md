...
1. **窄欄版面** — myON 選不起字，所以實際用法是 **iPad 分割畫面**
   （左邊 myON、右邊字典）。目前版面沒在窄欄測過
2. **語音輸入** — iOS 不支援 Web Speech API 的辨識，只能引導她用系統鍵盤的
   麥克風；桌機 Chrome 可以做真的按鈕
3. **作業容器** — 每次閱讀作業一組生字，紀錄才有意義、遊戲題庫才有範圍
4. **人工核心層** — 匯入教育部「國中小基本字彙」，用 `OVERRIDES` 機制
